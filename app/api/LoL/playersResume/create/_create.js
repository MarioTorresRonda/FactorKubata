...
export async function createPlayerResume() {
    const finalObj = {};
	...
	for await (const match of findResult) {
	...
	let playersResume = await getCollection("playersResume");
	await playersResume.insertOne( { resume: finalObj, date: new Date(), scrim: false } );
	await playersResume.insertOne( { resume: finalObjsScrims, date: new Date(), scrim: true } );
}

function statsObj({ player, scrim, matches }) {
